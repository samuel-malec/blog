...
A memory operation's ordering relative to another (when talking about ordering we mean ordering on the `successor` relation) is decided purely by whether their identities overlap. Two operations that touch disjoint location identities may be reordered or interleaved freely, but two operations that (might) share a location identity must stay ordered.
Currently one of the most apparent limitation is that an ordinary Java method call (`InvokeNode`) has no interprocedural information available to the compiler about what memory locations it touches, 
so it is constructed with `identity = LocationIdentity.any()`. This makes every call a `SingleMemoryKill` on `any()`, i.e. a full memory barrier.
To demonstrate, suppose this example: 
