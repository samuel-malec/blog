...
This is due to the sheer scope on which it has to operate, while dealing with all the related implementation details, 
...
In the following sections we will briefly introduce Native Image to the point where we are able to reason about some of its limitations that could be improved through our work. 
...
`LocationIdentity` only matter for floating memory accesses. Two floating accesses with disjoint identities can be scheduled independently of one another, whereas two accesses (potentially) sharing
...
To give concrete examples of this phenomenon, suppose the two use cases below, in both the compiler has enough information to know a call couldn't have touched a given location, but Graal IR didn't encode this information, so the optimization is blocked.

# Use Case I - Loop-Invariant Memory Access 
Consider:
```jsx
...
Ok, seeing the use cases, we will now demonstrate our proposal in its most general form.
Consider:
```text
operation A
    │
...
We shall be dealing with this question during the implementation...
